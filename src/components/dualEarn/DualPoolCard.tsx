...
export default function DualPoolCard({ stakingInfo, isOld }: { stakingInfo: DualStakingInfo, isOld: Boolean}) {
  ...
  //@ts-ignore
  const perMonthReturnInRewards: any = (((stakingInfo?.rateA * stakingInfo?.quickPrice) + (stakingInfo?.rateB * stakingInfo?.maticPrice)) * 30) / Number(valueOfTotalStakedAmountInUSDC?.toSignificant(6));
    ...
    <Wrapper showBackground={isStaking} bgColor={backgroundColor}>
      ...
      <TopSection>
        ...
        {isOld ? (
          ...
          <StyledInternalLink to={`/dual/${currencyId(currency0)}/${currencyId(currency1)}/${stakingInfo.stakingRewardAddress}`} style={{ width: '100%' }}>
            <ButtonPrimary padding="8px" borderRadius="8px">
              {isStaking ? 'Manage' : 'Deposit'}
            </ButtonPrimary>
          </StyledInternalLink>
      ...
      <StatContainer>
        ...
        <RowBetween>
          <TYPE.white> {rewardTokenA?.symbol} rate </TYPE.white>
          <TYPE.white>{`${stakingInfo.totalRewardRateA
        ...
        <>
          ...
          <BottomSection showBackground={true}>
            <TYPE.black color={'white'} fontWeight={500}>
              <span>Your {rewardTokenA?.symbol} rate</span>
            </TYPE.black>
            ...
            <TYPE.black style={{ textAlign: 'right' }} color={'white'} fontWeight={500}>
              ...
              {`${stakingInfo.rewardRateA
                ?.multiply(`${60 * 60 * 24}`)
                ?.toSignificant(4, { groupSeparator: ',' }) + " " + rewardTokenA?.symbol} / day`}
            </TYPE.black>
          </BottomSection>

          <BottomSection showBackground={true}>
            <TYPE.black color={'white'} fontWeight={500}>
              <span>Your {rewardTokenB?.symbol} rate</span>
            </TYPE.black>
            ...
            <TYPE.black style={{ textAlign: 'right' }} color={'white'} fontWeight={500}>
              ...
              {`${stakingInfo.rewardRateB
                ?.multiply(`${60 * 60 * 24}`)
                ?.toSignificant(4, { groupSeparator: ',' }) + " " + rewardTokenB?.symbol} / day`}
            </TYPE.black>
          </BottomSection>
