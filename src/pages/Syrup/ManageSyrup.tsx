...
export default function ManageSyrup({
  ...
  const baseTokenCurrency = unwrappedToken(syrupInfo?.baseToken ? syrupInfo?.baseToken : WETH[ChainId.MATIC]);
  ...
  const baseToken = baseTokenCurrency === empty ? tokenA: syrupInfo?.baseToken;
  
  const backgroundColor = useColor(tokenA)

  // get WETH value of staked LP tokens
  const totalSupplyOfStakingToken = useTotalSupply(syrupInfo?.stakedAmount?.token)
  let valueOfTotalStakedAmountInQuick: TokenAmount | undefined
  let valueOfMyStakedAmountInQUICK: TokenAmount | undefined

  if (totalSupplyOfStakingToken && syrupInfo && baseToken) {
    // take the total amount of LP tokens staked, multiply by QUICK value of all LP tokens, divide by all LP tokens
     // take the total amount of LP tokens staked, multiply by QUICK value of all LP tokens, divide by all LP tokens
     valueOfTotalStakedAmountInQuick = new TokenAmount(
      QUICK,
      JSBI.multiply(syrupInfo.totalStakedAmount.raw, syrupInfo.dQUICKtoQUICK.raw)
    )
    valueOfMyStakedAmountInQUICK = new TokenAmount(
      QUICK,
      JSBI.multiply(syrupInfo.stakedAmount.raw, syrupInfo.dQUICKtoQUICK.raw)
    )
  }
        ...
        <PoolData>
          <AutoColumn gap="sm">
            ...
            {valueOfTotalStakedAmountInUSDC
                ...
                : `${valueOfTotalStakedAmountInQuick?.toSignificant(4, { groupSeparator: ',' }) ?? '-'} QUICK`}
            </TYPE.body>
          </AutoColumn>
        </PoolData>
        <PoolData>
      ...
      {showAddLiquidityButton && (
        <VoteCard>
          ...
          <CardSection>
            <AutoColumn gap="md">
              <RowBetween>
      <TYPE.white fontWeight={600}>Step 1. Get {(syrupInfo?.name && syrupInfo?.name !== "" ? syrupInfo.name : "QUICK-V2")} Liquidity tokens</TYPE.white>
              </RowBetween>
              <RowBetween style={{ marginBottom: '1rem' }}>
                <TYPE.white fontSize={14}>
                  dQUICK tokens are required
                </TYPE.white>
              </RowBetween>
            </AutoColumn>
          </CardSection>
          ...
          <StyledDataCard disabled={disableTop} bgColor={backgroundColor} showBackground={!showAddLiquidityButton}>
            <CardSection>
              ...
              <AutoColumn gap="md">
                ...
                : `${valueOfMyStakedAmountInQUICK?.toSignificant(4, { groupSeparator: ',' }) ?? '-'}`}
                  </TYPE.white>
                  <TYPE.white>
                    dQUICK
                  </TYPE.white>
